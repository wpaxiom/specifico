        ...
        <>
            {metaFields.map((field, index) => (
                <div key={index} className="grid gap-3.5 items-center px-6 py-4 border-b border-[#F3F3F8]" style={{ gridTemplateColumns: MAP_GRID }}>
                    <MultiSelect bare placeholder={ __( 'Select specification', 'specifico' ) } value={field.category} onChange={e => onChange(index, 'category', e)} options={spec}/>
                    <MultiSelect bare placeholder={ __( 'Select type', 'specifico' ) } value={field.type} onChange={e => onChangeType(index, 'type', e)} options={type}/>
                    <MultiSelect bare isMulti placeholder={ __( 'Select values', 'specifico' ) } value={field.values} onChange={e => onChange(index, 'values', e)} options={ optionsByType[ field.type?.value ] || [] } isDisabled={ ! field.type?.value }/>
                    <button type="button" onClick={() => onRemoveField(index)} aria-label={ __( 'Remove mapping', 'specifico' ) } className="inline-flex items-center justify-center w-[38px] h-[38px] border border-[#F4DADA] bg-[#FEF6F6] rounded-[10px] hover:bg-[#FCEAEA] transition-colors">
            ...
            <div className="px-6 py-[18px]">
                <button type="button" onClick={onAddField} className="inline-flex items-center gap-[7px] h-[38px] px-4 bg-[#EDEBFF] text-[#6B66F7] border-none rounded-[10px] font-bold text-[13.5px] cursor-pointer hover:bg-[#E2DFFF] transition-colors">
                    <span className="text-[17px] leading-none -mt-px">+</span> { __( 'Add Mapping', 'specifico' ) }
                </button>
            </div>
        </>
    );
