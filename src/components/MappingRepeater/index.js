        ...
        <>
            {metaFields.map((field, index) => (
                <div key={index} className="flex gap-2.5 relative before:content-[''] before:w-full before:h-px before:bg-dash-border before:bg-repeat-x before:absolute before:bottom-0">
                    ...
                    <MultiSelect isMulti placeholder="Select values" value={field.values} hasLabel={false} onChange={e => onChange(index, 'values', e)} options={ optionsByType[ field.type?.value ] || [] } isDisabled={ ! field.type?.value }/>
