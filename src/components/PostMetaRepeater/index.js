...
const ATTRIBUTE_TYPES = [
    { label: __( 'Text', 'specifico' ), value: "text" },
    { label: __( 'Select', 'specifico' ), value: "select" },
    { label: __( 'Radio', 'specifico' ), value: "radio" },
    { label: __( 'Textarea', 'specifico' ), value: "textarea" },
];
        ...
        <>
            {metaFields.map((field, index) => (
                <div key={index} className="grid grid-cols-[1fr_1fr_1fr_48px] gap-3.5 items-center px-6 py-3.5 border-b border-[#F3F3F8]">
                    <input type="text" value={field.attributeName} onChange={e => onChange(index, 'attributeName', e.target.value)} placeholder={ __( 'Attribute name', 'specifico' ) } className={FIELD} />
                    <input type="text" value={field.attributeValue} onChange={e => onChange(index, 'attributeValue', e.target.value)} placeholder={ __( 'Attribute value', 'specifico' ) } className={FIELD} />
                    <div className="flex flex-col gap-2">
                        ...
                        {(field.attributeType === 'select' || field.attributeType === 'radio') && (
                            <input type="text" value={field.defaultValue} onChange={e => onChange(index, 'defaultValue', e.target.value)} placeholder={ __( 'Default value', 'specifico' ) } className={FIELD} />
                        )}
                    </div>
                    <button type="button" onClick={() => onRemoveField(index)} aria-label={ __( 'Remove attribute', 'specifico' ) } className="inline-flex items-center justify-center w-[38px] h-[38px] border border-[#F4DADA] bg-[#FEF6F6] rounded-[10px] hover:bg-[#FCEAEA] transition-colors">
            ...
            <div className="px-6 py-[18px]">
                <button type="button" onClick={onAddField} className="inline-flex items-center gap-[7px] h-[38px] px-4 bg-[#EDEBFF] text-[#6B66F7] border-none rounded-[10px] font-bold text-[13.5px] cursor-pointer hover:bg-[#E2DFFF] transition-colors">
                    <span className="text-[17px] leading-none -mt-px">+</span> { __( 'Add Attribute', 'specifico' ) }
                </button>
            </div>
        </>
    );
